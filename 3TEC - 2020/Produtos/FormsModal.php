<div class="modal fade" id="FormLogin" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body">
                <form class="" action="Login.php" method="POST">

                    <div class="card">
                        <div class="card-header BarraTopo text-center">
                            <h5 class="card-title text-white">Área Administrativa</h5>
                        </div>
                        <div class="card-body text-center">
                            Informe abaixo os dados de login.
                            <br><br>


                            <div class="form-group">
                                <input type="text" class="form-control" name="HTML_LOGIN" placeholder="Nome de Usuário">
                            </div>
                            <div class="form-group">
                                <input type="password" class="form-control" name="HTML_SENHA" placeholder="Senha">
                            </div>
                            <div class="form-group">
                                <button class="btn float-right Botao">Acessar área</button>
                            </div>


                        </div>
                    </div>


                </form>
            </div>
        </div>                
    </div> 
</div>






<div class="modal fade" id="FormCadastroUsuario" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-body">
                <form class="" action="CadastroUsuario.php" method="POST">

                    <div class="card">
                        <div class="card-header BarraTopo text-center">
                            <h5 class="card-title text-white">Cadastro de Usuário</h5>
                        </div>
                        <div class="card-body text-center">
                            Informe abaixo os dados para cadastro.
                            <br><br>


                            <div class="form-group">
                                <input type="text" class="form-control" name="HTML_LOGIN" placeholder="Nome de Usuário">
                            </div>
                            <div class="form-group">
                                <input type="password" class="form-control" name="HTML_SENHA" placeholder="Senha">
                            </div>
                            <div class="form-group">
                                <input type="password" class="form-control" name="HTML_CONFSENHA" placeholder="Confirme sua Senha">
                            </div>
                            <div class="form-group">
                                <button class="btn float-right Botao">Cadastrar</button>
                            </div>


                        </div>
                    </div>


                </form>
            </div>
        </div>                
    </div> 
</div>